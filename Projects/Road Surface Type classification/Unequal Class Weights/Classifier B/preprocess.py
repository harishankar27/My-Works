...
train_data = np.zeros((n_data_points_train,300,7))
...
for i in train_set:
    ...
    for j in range(0, n):
        ...
        train_data[n_window,:,:] = normalized_window
        n_window = n_window + 1
...
test_data = np.zeros((n_data_points_test,300,7))
...
for i in test_set:
    ...
    for j in range(0, n):
        window = df.iloc[j*overlap_size:j*overlap_size + window_size]
        normalized_window = pd.DataFrame(scaler.fit_transform(window), columns=window.columns)
        test_data[n_window,:,:] = normalized_window
        n_window = n_window + 1
